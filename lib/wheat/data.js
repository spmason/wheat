...
var Data = module.exports = {
  ...
  // Loads the core data for all the articles and sorts the list by date.
  taggedArticles: Git.safe(function articles(version, tag, callback) {
    Step(
      function getListOfArticles() {
        Git.readDir(version, "articles", this);
      },
      ...
      function sortAndFinish(err, articles) {
        if (err) { callback(err); return; }
        articles = articles.filter(function tagFilter(article){
            return (article.tags || '').split(' ').indexOf(tag) > -1;
          }).sort(function dateSorter(a, b) {
            return (Date.parse(b.date)) - (Date.parse(a.date));
          });
        return articles;
      },
  ...
  // Loads the core data for all the tags and sorts the list by tagName.
  tags: Git.safe(function tags(version, callback) {
    Step(
      function getListOfArticles() {
        Git.readDir(version, "articles", this);
      },
      function readArticleTags(err, results) {
        if (err) { callback(err); return; }
        var group = this.group();
        results.files.forEach(function onFile(filename) {
          if (!(/\.markdown$/.test(filename))) {
      ...
      function pullOutTags(err, articles){
        if (err) { callback(err); return; }
        var tags = [];
        articles.forEach(function getTags(article){
          (article.tags || '').split(' ').forEach(function addTag(tag){
            if(tags.indexOf(tag) == -1)
              tags.push(tag);
          });
        });
        tags = tags.filter(function(a){return a !== '';})
          .sort(function tagSorter(a, b) {
            return a > b;
          });
        return tags;
      },
